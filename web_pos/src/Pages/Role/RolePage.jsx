...
const RolePage = () => {
  ...
  const getLists = async () => {
    ...
    const res = await request('role'+queryparams, 'get'); //localhost:8000/api/role?page=1&text_search=admin
  ...
  const onDelete = async (data) => {
    Modal.confirm({
      title : "Delete",
      content : `Are you sure to delete this ${data.name} role?`,
      onOk : async () => {
  ...
  return (
    <Spin spinning={state.loading}>
      <Space style={{display:"flex", justifyContent:"space-between"}}>
        <Space>
          <div>Total Role: {state.list.length}</div>
      ...
      <Modal
        ...
        onCancel={closeModal}
        footer={null}
        title={formRef.getFieldValue('id')? "Edit Role" : "Add Role"}
      >
